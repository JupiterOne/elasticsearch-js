    ...
    async function iterate () {
      ...
      for await (const chunk of datasource) {
        ...
        if (operation === 'index' || operation === 'create') {
          const hasOverride = Array.isArray(action)
          actionBody = hasOverride ? serializer.serialize(action[0]) : serializer.serialize(action)
          payloadBody = hasOverride ? serializer.serialize(action[1]) : typeof chunk === 'string' ? chunk : serializer.serialize(chunk)
